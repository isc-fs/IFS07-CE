...
FDCAN_TxHeaderTypeDef TxHeader_Hub;
FDCAN_RxHeaderTypeDef RxHeader_Hub;

uint8_t TxData_Inv[8];
uint8_t RxData_Inv[8];
uint8_t TxData_Acu[8];
uint8_t RxData_Acu[8];
uint8_t TxData_Hub[8];
uint8_t RxData_Hub[8];
...
void SDCard_write(char *filename, char *data);
...
//							byte1_voltage = RxData_Inv[1];
			...
			if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader_Acu,
				...
				case 0x111:
					...
					break;
					//s2_aceleracion = ((int)RxData_Acu[2] << 8) | RxData_Acu[3]; // juntamos los bytes 3 y 4 del envío del HUB
				}
			}
			/*
			else if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader_Hub, RxData_Hub) == HAL_OK){
				if (RxHeader_Hub.Identifier == 0x111){ // señal enviada por la HUB a la ECU con los datos de aceleración
